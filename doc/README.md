...
* Introduction                                   *
**************************************************
This file should contain a general introduction to modifying Bomba:

The general file structure, where to find/place code, tests and documentation.

How to build Bomba, including prerequisites.
...
* Building Bomba                                  *
**************************************************
To build Bomba should be a simple step. Just navigate to the root folder for this project and type 'make bomba' (and do your normal Voodoo dance that you do to get an accepted submission). If the build process fails, then it's time to start improving your Voodoo dance, but in the meantime, have a look the debug section below.
...
Second, you can run the code processor outside of the build process, and check the result if it contains any obvious errors. The code processor is the python program build/bombaprocessor.py.
Ex. python bombaprocessor.py -i my_non_working_source_file.h will run the code processor in the same way as the build process does.
...
Finally, you'll need to modify the file build/bomba.tex to add the new chapter folder. Notice that if you've followed the instructions above, all you need is to specify the name of the new chapter folder with the 'bombachapter' command.
Ex. \bombachapter{geometry} will include the file content/geometry/chapter.tex
...
Ex. \bombaimport{example.h} will include the file example.h from the same folder.
...
**************************************************
* Modifying the Bomba edition                    *
**************************************************
The edition name and date can be modified from the file build/bomba.tex, and are set with the commands \date{2009-04-11} respectively \edition{Edition name}.
